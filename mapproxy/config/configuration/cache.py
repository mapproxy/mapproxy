...
class CacheConfiguration(ConfigurationBase):
    ...
    def cache_dir(self):
        ...
        if cache_dir:
            if self.conf.get("cache_dir"):
                log.warning(
                    "found cache.directory and cache_dir option for %s, ignoring cache_dir",
                    self.conf["name"],
                )
            return self.context.globals.abspath(cache_dir)

        return self.context.globals.get_path(
            "cache_dir", self.conf, global_key="cache.base_dir"
        )
    ...
    def directory_permissions(self):
        ...
        if directory_permissions:
            log.info(
                "Using cache specific directory permission configuration for %s: %s",
                self.conf["name"],
                directory_permissions,
            )
            return directory_permissions

        global_permissions = self.context.globals.get_value(
            "directory_permissions", self.conf, global_key="cache.directory_permissions"
        )
        if global_permissions:
            log.info(
                "Using global directory permission configuration for %s: %s",
                self.conf["name"],
                global_permissions,
            )
    ...
    def file_permissions(self):
        ...
        if file_permissions:
            log.info(
                "Using cache specific file permission configuration for %s: %s",
                self.conf["name"],
                file_permissions,
            )
        ...
        global_permissions = self.context.globals.get_value(
            "file_permissions", self.conf, global_key="cache.file_permissions"
        )
        if global_permissions:
            log.info(
                "Using global file permission configuration for %s: %s",
                self.conf["name"],
                global_permissions,
            )
        return global_permissions
    ...
    def _file_cache(self, grid_conf, image_opts):
        ...
        link_single_color_images = self.context.globals.get_value(
    ...
    def _geopackage_cache(self, grid_conf, image_opts):
        ...
        filename = self.conf["cache"].get("filename")
        table_name = self.conf["cache"].get("table_name") or "{}_{}".format(
            self.conf["name"], grid_conf.tile_grid().name
        )
        ...
        if cache_dir:
            cache_dir = os.path.join(
                self.context.globals.abspath(cache_dir), grid_conf.tile_grid().name
            )
        else:
            cache_dir = self.cache_dir()
            cache_dir = os.path.join(
    ...
    def _azureblob_cache(self, grid_conf, image_opts):
        ...
        container_name = self.context.globals.get_value(
        ...
        if not container_name:
            raise ConfigurationError(
                "no container_name configured for Azure Blob cache %s"
                % self.conf["name"]
            )

        connection_string = os.getenv(
            "AZURE_STORAGE_CONNECTION_STRING",
            self.context.globals.get_value(
                "cache.connection_string",
                self.conf,
                global_key="cache.azureblob.connection_string",
            ),
        )

        if not connection_string:
            raise ConfigurationError(
                "no connection_string configured for Azure Blob cache %s"
                % self.conf["name"]
            )
    ...
    def _s3_cache(self, grid_conf, image_opts):
        ...
        bucket_name = self.context.globals.get_value(
            "cache.bucket_name", self.conf, global_key="cache.s3.bucket_name"
        )
        coverage = self.coverage()

        if not bucket_name:
            raise ConfigurationError(
                "no bucket_name configured for s3 cache %s" % self.conf["name"]
            )

        profile_name = self.context.globals.get_value(
            "cache.profile_name", self.conf, global_key="cache.s3.profile_name"
        )

        region_name = self.context.globals.get_value(
            "cache.region_name", self.conf, global_key="cache.s3.region_name"
        )

        endpoint_url = self.context.globals.get_value(
            "cache.endpoint_url", self.conf, global_key="cache.s3.endpoint_url"
        )

        access_control_list = self.context.globals.get_value(
            "cache.access_control_list",
            self.conf,
            global_key="cache.s3.access_control_list",
        )

        use_http_get = self.context.globals.get_value(
            "cache.use_http_get", self.conf, global_key="cache.s3.use_http_get"
        )

        include_grid_name = self.context.globals.get_value(
            "cache.include_grid_name",
            self.conf,
    ...
    def _tile_filter(self):
        ...
        if "watermark" in self.conf:
            ...
            if self.conf["watermark"].get("color"):
                self.conf["watermark"]["color"] = parse_color(
                    self.conf["watermark"]["color"]
                )
    ...
    def image_opts(self):
        ...
        image_opts = self.context.globals.image_options.image_opts(
            self.conf.get("image", {}), format
        )
    ...
    def supports_tiled_only_access(self, params=None, tile_grid=None):
        ...
        if (
            tile_grid.is_subset_of(cache_grid)
    ...
    def _sources_for_band_merge(self, sources_conf, grid_conf, request_format):
        ...
        for band, band_sources in sources_conf.items():
            ...
            if band_idx is None:
                raise ConfigurationError(
                    "unsupported band '%s' for cache %s" % (band, self.conf["name"])
                )
            for source in band_sources:
                band_merger.add_ops(
                    dst_band=band_idx,
    ...
    def caches(self):
        ...
        if self.conf.get("format") == "mixed" and self.conf.get(
            "request_format"
        ) not in ["image/png", "image/vnd.jpeg-png"]:
            raise ConfigurationError(
        ...
        meta_buffer = self.context.globals.get_value(
            "meta_buffer", self.conf, global_key="cache.meta_buffer"
        )
        meta_size = self.context.globals.get_value(
            "meta_size", self.conf, global_key="cache.meta_size"
        )
        bulk_meta_tiles = self.context.globals.get_value(
            "bulk_meta_tiles", self.conf, global_key="cache.bulk_meta_tiles"
        )
        minimize_meta_requests = self.context.globals.get_value(
            "minimize_meta_requests",
            self.conf,
            global_key="cache.minimize_meta_requests",
        )
        concurrent_tile_creators = self.context.globals.get_value(
        ...
        if upscale_tiles and downscale_tiles:
            raise ConfigurationError(
                "cannot use both upscale_tiles and downscale_tiles"
            )
        ...
        for grid_name, grid_conf in self.grid_confs():
            ...
            image_opts = compatible_image_options(
                source_image_opts, base_opts=base_image_opts
            )
            ...
            if use_renderd:
                from mapproxy.cache.renderd import (
                    RenderdTileCreator,
                    has_renderd_support,
                )
                ...
                global_directory_permissions = self.context.globals.get_value(
                    "directory_permissions",
                    self.conf,
                ...
                if global_directory_permissions:
                    log.info(
                        f"Using global directory permission configuration for tile locks:"
                        f" {global_directory_permissions}"
                    )

                global_file_permissions = self.context.globals.get_value(
                    "file_permissions", self.conf, global_key="cache.file_permissions"
                )
                if global_file_permissions:
                    log.info(
                        f"Using global file permission configuration for tile locks:"
            ...
            if isinstance(cache, DummyCache):
                ...
                locker = TileLocker(
                    ...
                    lock_timeout=self.context.globals.get_value(
                        "http.client_timeout", {}
                    ),
            ...
            if self.conf["name"] in self.context.caches:
                mgr._refresh_before = self.context.caches[self.conf["name"]].conf.get(
                    "refresh_before", {}
                )
    ...
    def map_layer(self):
        ...
        image_opts = self.image_opts()
        max_tile_limit = self.context.globals.get_value(
            "max_tile_limit", self.conf, global_key="cache.max_tile_limit"
        )
        caches = []
        main_grid = None
        ...
        if "use_direct_from_level" in self.conf:
            self.conf["use_direct_from_res"] = main_grid.resolution(
                self.conf["use_direct_from_level"]
            )
        if "use_direct_from_res" in self.conf:
            if len(self.conf["sources"]) != 1:
                raise ValueError(
                    "use_direct_from_level/res only supports single sources"
                )
            source_conf = self.context.sources[self.conf["sources"][0]]
            layer = ResolutionConditional(
                layer,
                source_conf.source(),
                self.conf["use_direct_from_res"],
                main_grid.srs,
                layer.extent,
                opacity=image_opts.opacity,
            )
